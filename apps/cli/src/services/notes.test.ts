...
describe('a heading written above its frontmatter', () => {
  const raw = [
    '# 잊지 않는 존재의 기억 정리법',
    '',
    '---',
    'title: 잊지 않는 존재의 기억 정리법',
    'tags: [기억]',
    '---',
    '',
    '이번 포스팅에서는.',
  ...
  it('keeps the metadata out of the body whichever order it arrived in', () => {
    expect(bodyOf(raw, '잊지 않는 존재의 기억 정리법')).toBe('이번 포스팅에서는.');
  });

  it('puts the same head back when the body is saved', () => {
    const body = bodyOf(raw, '잊지 않는 존재의 기억 정리법');
    expect(recompose(raw, body, '잊지 않는 존재의 기억 정리법')).toBe(raw);
  });
});
