...
const embedder = async () => new Array(768).fill(0);
...
describe('a correction saved through the note service', () => {
  // `corrects` is the stronger claim, so it is written only when the caller
  // asks for it by name. A save that only says `amends` gets the weaker edge,
  // which is what an agent recording a follow-up gets by default.
  it('says corrects when the caller named it', async () => {
    const older = addNote('what was recorded');

    await saveNote(client, embedder, dbDir, {
