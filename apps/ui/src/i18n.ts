...
const en = {
  ...
  chores: {
    ...
    startRepair: 'Work through them →',
  },
  repair: {
    title: 'Repair session',
    subtitle: 'One judgement at a time. It ends when the stack does.',
    progress: (done: number, size: number) => `${done} of ${size}`,
    remaining: (n: number) => `${n} left after this stack`,
    empty: 'Nothing here needs a source right now.',
    sources: 'Which of these is it built on?',
    declare: 'Declare and continue',
    none: 'No sources',
    skip: 'Skip',
    leave: 'Leave',
    keys: '1-9 pick · enter declare · n none · s skip · esc leave',
    doneTitle: (n: number) => `${n} judgements now say where they came from.`,
    again: 'One more stack',
...
const ko: typeof en = {
  ...
  chores: {
    ...
    openTags: '태그 목록 열기 →',
    startRepair: '한 판 정리하기 →',
  },
  repair: {
    title: '정비 세션',
    subtitle: '한 번에 판단 하나. 묶음이 끝나면 세션도 끝나.',
    progress: (done, size) => `${size}장 중 ${done}장`,
    remaining: (n) => `이 묶음 뒤로 ${n}개 남음`,
    empty: '지금 근거를 물어볼 판단이 없어.',
    sources: '이 판단은 무엇을 딛고 서 있어?',
    declare: '근거로 선언하고 계속',
    none: '근거 없음',
    skip: '건너뛰기',
    leave: '나가기',
    keys: '1-9 선택 · enter 선언 · n 근거없음 · s 건너뛰기 · esc 나가기',
    doneTitle: (n) => `판단 ${n}개가 출처를 말하게 됐어.`,
    again: '한 판 더',
    home: '개요로 돌아가기',
  },
  overview: {
  ...
  note: {
    viewSource: '원본 보기',
    hideSource: '원본 닫기',
    sourceMissing: '색인에는 있는데 그 경로에 파일이 없어.',
    agent: '에이전트',
  ...
  edit: {
    ...
    missingWhy: '이 이름으로 열리는 노트가 볼트에 없어.',
    createNote: '노트 저장',
