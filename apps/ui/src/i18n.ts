...
const en = {
  ...
  app: {
    ...
    refresh: 'Reload',
    overview: 'Overview',
    deck: 'What the AI is claiming',
    staleServer: 'This window is newer than the app serving it. Quit memex and open it again.',
  ...
  error: (failure: ApiFailure) => {
    const messages: Record<string, string> = {
      ...
      'draft-provider-missing': 'That AI provider is not installed on this computer.',
      'draft-logged-out': 'Sign in to that AI provider, then try again.',
      'draft-quota': 'That AI provider has reached its usage limit.',
      'draft-model-refused': 'That model is not available for this account. Choose another model.',
      'draft-timeout': 'The model stopped responding. Try again or choose another model.',
      'draft-cancelled': 'Drafting stopped.',
      'draft-unreadable': 'The model did not return a usable draft. Try again.',
      'draft-failed': 'Could not create the draft. Try again or choose another model.',
      'empty-body': 'There is nothing under the title yet.',
      'save-rejected': 'That note could not be saved as it is.',
  ...
  deck: {
    title: 'What the AI is claiming',
    progress: (n: number, total: number) => `${n} / ${total}`,
    loading: 'Reading what the AI holds…',
    errorTitle: 'Could not load the deck.',
    emptyTitle: 'Nothing needs your judgement.',
    emptyLead:
      'Memories have been added since you last looked, but everything the AI is claiming still holds.',
    doneTitle: 'That is this session.',
    doneLead: 'Seven is a session. Come back tomorrow, or keep going.',
    more: 'Show me more',
    kindClaim: 'The AI is claiming',
    kindRule: 'A rule waiting for you',
    kindInference: 'A hypothesis the AI drew',
    since: (date: string) => `from ${date}`,
    idle: (n: number) => `${n} days unchecked`,
    neverChecked: 'never checked',
    spoken: (n: number, days: number) => `Said ${n} times in conversation in the last ${days} days`,
    spokenNone: 'Not said in conversation yet',
    moved: 'Its source moved',
    movedLead: 'The note this was read out of no longer says what it said.',
    evidence: 'Evidence',
    notAFact: 'Not a fact to check',
    notAFactDone: 'Taken out. This one will not be asked again.',
    confirm: 'Still right',
    confirmRule: 'Let it in',
    confirmInference: 'Still holds',
    correctSoon: 'Marked as wrong. Putting in the new value comes next.',
    ruleCost: 'Approving starts feeding this rule back to the AI every conversation.',
    defer: 'Later',
    undo: 'Undo',
    undone: 'Taken back.',
    confirmedCard: 'Marked right. Back in 30 days.',
    approvedRule: 'Let in. The AI reads it from the next conversation.',
    confirmedEvidence: 'Marked right against its evidence. Back in 90 days.',
    downgraded: 'Third session today, so this one comes back in 30 days.',
    deferred: 'Set aside.',
    correct: 'It changed',
    keys: '← changed · ↓ later · → still right · Space evidence · ⌘Z undo',
    actionFailed: 'That did not go through.',
  },
  review: {
    title: 'What needs your judgement',
    progress: (n: number, total: number) => `${n} / ${total}`,
    loading: 'Looking through what changed…',
    errorTitle: 'Could not load what needs checking.',
    emptyTitle: 'Nothing needs your judgement.',
    emptyLead:
      'Memories have been added since you last looked, but the current beliefs and rules still hold.',
    kindCorrected: 'A source was corrected',
    kindMoved: 'A source moved',
    gradeObserved: 'observed',
    gradeInferred: 'AI inference',
    recurring: 'again',
    recurringHint: 'You stood behind this one before, and its sources moved again.',
    knows: 'What the AI holds',
    changed: 'What changed',
    consequence: 'If you leave it',
    consequenceCorrected: 'This claim keeps being used in conversation as it stands.',
    consequenceMoved: 'This hypothesis stays on record while its sources say something else.',
    injected: (n: number, days: number) => `Used ${n} times in the last ${days} days.`,
    injectedNone: 'Not used in conversation yet.',
    correctedBy: (title: string) => `Corrected by “${title}”`,
    retired: 'was',
    before: 'then',
    beforeMissing: 'What it said then was not kept — git still has it.',
    now: 'now',
    stateChanged: 'changed',
    stateCorrected: 'corrected',
    stateMissing: 'gone',
    stateIntact: 'unchanged',
    sources: (n: number) => `${n} sources`,
    approve: 'Still right',
    approveCost: 'Approving marks this claim as checked, and it keeps being used as it is.',
    edit: 'Fix it',
    defer: 'Later',
    deferHint: 'It comes back if the AI starts using it, or if its sources move again.',
    remove: 'Remove this',
    removeNote: 'Remove this memory',
    removeInference: 'Remove this hypothesis',
    removeConfirm: 'Remove it? The note file is deleted — git still has it.',
    removeConfirmInference: 'Remove it? It stops being read anywhere.',
    removeYes: 'Remove',
    removeNo: 'Keep',
    missingTitle: 'Could not read the sources for this one.',
    missingLead: 'The file may have been moved or deleted.',
    reveal: 'Open where it should be',
    actionFailed: 'That did not go through.',
    settledApproved: 'Marked as still right.',
    settledDeferred: 'Set aside.',
    settledRemoved: 'Removed.',
    done: 'That is everything for now.',
  },
  today: {
  ...
  settings: {
    ...
    appearanceHint: 'Choose whether memex follows this computer or keeps one theme.',
    system: 'Use system setting',
    light: 'Light',
    dark: 'Dark',
    language: 'Language',
    groups: {
      appearance: { title: 'Appearance & language', hint: 'How memex looks and reads.' },
      notes: { title: 'Notes & search', hint: 'Where your notes live and how they are indexed.' },
      ai: { title: 'AI', hint: 'Who handles each kind of AI work.' },
      connections: { title: 'Connections', hint: 'Apps that can read from or write to memex.' },
      data: { title: 'Data & maintenance', hint: 'Inspect and repair the local vault.' },
    },
    jobs: {
    ...
    sources: 'Vaults memex reads',
    sourcesHint:
      'Your main vault is where new notes are saved. Add other Markdown folders to search them together.',
    primaryVault: 'New notes are saved here',
    connectedVault: 'Connected vault',
    addVault: 'Add a vault',
    disconnectVault: 'Disconnect',
    disconnectHint: 'Files stay in their folder. memex only removes them from search.',
    reindex: 'Reindex',
    reindexing: 'Reindexing…',
    indexDone: (changed: number, unchanged: number) =>
      `${changed} changed · ${unchanged} unchanged`,
    sourceFailed: 'Could not change this vault. Check that the folder is still available.',
    ...
    vaultHint: 'Lists and tidying that live behind the three things above.',
  ...
  references: {
    ...
    nothingFound: 'Nothing matched.',
    changed: 'What you quoted has changed since.',
  ...
  edit: {
    ...
    draftModel: 'Model for this draft',
...
const ko: typeof en = {
  ...
  app: {
    ...
    overview: '확인하기',
    deck: '확인하기',
    staleServer: '앱을 다시 켜 주세요. 화면이 지금 도는 앱보다 최신이에요.',
  ...
  error: (failure) => {
    const messages: Record<string, string> = {
      'not-found': '없는 노트예요.',
      'draft-state-only': '현재 믿음 노트만 갱신 초안을 만들 수 있어요.',
      'edit-rejected': '여기서는 고칠 수 없는 노트예요.',
      'draft-no-evidence': '갱신할 근거 노트가 없어요.',
      'draft-no-claude': 'Claude Code를 찾을 수 없어요. 먼저 설치해 주세요.',
      'draft-provider-missing': '이 컴퓨터에 해당 AI 제공자가 설치되어 있지 않아요.',
      'draft-logged-out': '해당 AI 제공자에 로그인한 뒤 다시 시도해 주세요.',
      'draft-quota': '해당 AI 제공자의 사용 한도에 도달했어요.',
      'draft-model-refused': '이 계정에서는 해당 모델을 사용할 수 없어요. 다른 모델을 골라 주세요.',
      'draft-timeout': '모델 응답이 멈췄어요. 다시 시도하거나 다른 모델을 골라 주세요.',
      'draft-cancelled': '초안 작성을 중단했어요.',
      'draft-unreadable': '모델이 사용할 수 있는 초안을 만들지 못했어요. 다시 시도해 주세요.',
      'draft-failed': '초안을 만들지 못했어요. 다시 시도하거나 다른 모델을 골라 주세요.',
      'empty-body': '제목 아래에 아직 아무것도 없어요.',
      'save-rejected': '이 상태로는 저장할 수 없어요.',
      ...
      unreachable: 'memex에 연결할 수 없어요. 앱을 다시 켜 보세요.',
    };
  ...
  thisWeek: {
    ...
    connection: '오래된 노트와 이어진 것',
    connectionNone: '오늘은 오래된 노트와 이어진 게 없어요',
    apart: (days) => `${days}일 차이`,
  },
  threads: {
    title: '정정 흐름',
    lead: '한 노트가 어떻게 고쳐져 왔는지, 어디서 두 갈래로 나뉘었는지 보여줘요.',
  ...
  notFound: {
    title: '없는 페이지',
    lead: '이 주소가 가리키는 노트가 없어요.',
    home: '확인하기로 돌아가기',
  },
  deck: {
    title: '확인하기',
    progress: (n, total) => `${n} / ${total}`,
    loading: 'AI가 아는 것을 읽고 있어요…',
    errorTitle: '확인할 것을 불러오지 못했어요.',
    emptyTitle: '지금 확인할 것은 없어요',
    emptyLead: '그동안 기억은 늘었지만, AI가 아는 것 중 확인이 필요한 건 없어요.',
    doneTitle: '오늘 몫은 여기까지예요',
    doneLead: '한 번에 일곱 장이에요. 내일 다시 오셔도 되고, 더 보셔도 돼요.',
    more: '더 볼래요',
    kindClaim: 'AI가 이렇게 알고 있어요',
    kindRule: '승인을 기다리는 규칙',
    kindInference: 'AI가 세운 가설',
    since: (date) => `${date}부터`,
    idle: (n) => `${n}일째 확인 안 됨`,
    neverChecked: '아직 확인 안 됨',
    spoken: (n, days) => `지난 ${days}일 동안 대화에서 ${n}번 썼어요`,
    spokenNone: '아직 대화에서 쓰이지 않았어요',
    moved: '근거가 움직였어요',
    movedLead: '이 말을 뽑아낸 노트가 지금은 다르게 쓰여 있어요.',
    evidence: '근거',
    notAFact: '판정할 게 아니에요',
    notAFactDone: '뺐어요. 이건 다시 안 물어볼게요.',
    confirm: '아직 맞아요',
    confirmRule: '승인할게요',
    confirmInference: '아직 맞아요',
    correctSoon: '틀렸다고 표시했어요. 맞는 값은 나중에 넣을 수 있어요.',
    ruleCost: '승인하면 이 규칙이 다음 대화부터 AI에게 전달돼요.',
    defer: '나중에',
    undo: '되돌리기',
    undone: '되돌렸어요.',
    confirmedCard: '맞다고 확인했어요. 30일 뒤에 다시 여쭤볼게요.',
    approvedRule: '승인했어요. 다음 대화부터 AI가 읽어요.',
    confirmedEvidence: '근거까지 보고 확인했어요. 90일 뒤에 다시 여쭤볼게요.',
    downgraded: '오늘 세 번째 세션이라 이건 30일 뒤에 다시 올라와요.',
    deferred: '나중에 다시 보여드릴게요.',
    correct: '바뀌었어요',
    keys: '← 바뀌었어요 · ↓ 나중에 · → 아직 맞아요 · Space 근거 · ⌘Z 되돌리기',
    actionFailed: '처리하지 못했어요.',
  },
  review: {
    title: '지금 확인할 것',
    progress: (n, total) => `${n} / ${total}`,
    loading: '무엇이 바뀌었는지 살펴보고 있어요…',
    errorTitle: '확인할 것을 불러오지 못했어요.',
    emptyTitle: '지금 확인할 것은 없어요',
    emptyLead: '그동안 기억은 늘었지만, 현재 믿음과 규칙은 그대로 안전해요.',
    kindCorrected: '근거가 정정됐어요',
    kindMoved: '근거가 움직였어요',
    gradeObserved: '관측',
    gradeInferred: 'AI 추론',
    recurring: '재발',
    recurringHint: '전에 한 번 확인한 기억인데 근거가 또 움직였어요.',
    knows: 'AI가 이렇게 알고 있어요',
    changed: '무엇이 바뀌었나',
    consequence: '그대로 두면',
    consequenceCorrected: '이 주장이 다음 대화에서 지금 모습 그대로 쓰여요.',
    consequenceMoved: '근거와 어긋난 가설이 그대로 남아요.',
    injected: (n, days) => `지난 ${days}일 동안 ${n}번 쓰였어요.`,
    injectedNone: '아직 대화에서 쓰이지 않았어요.',
    correctedBy: (title) => `「${title}」이 정정했어요`,
    retired: '이전',
    before: '그때',
    beforeMissing: '그때 내용은 저장돼 있지 않아요.',
    now: '지금',
    stateChanged: '바뀜',
    stateCorrected: '정정됨',
    stateMissing: '사라짐',
    ...
    sources: (n) => `근거 ${n}건`,
    approve: '맞아요',
    approveCost: '승인하면 이 주장이 확인된 것으로 표시되고 다음 대화에 그대로 쓰여요.',
    edit: '고칠게요',
    defer: '나중에',
    deferHint: 'AI가 이 기억을 다시 쓰거나 근거가 또 움직이면 다시 보여드려요.',
    remove: '지우기',
    removeNote: '이 기억 지우기',
    removeInference: '이 가설 지우기',
    removeConfirm: '지울까요? 노트 파일이 지워져요.',
    removeConfirmInference: '지울까요? 이후로는 어디에서도 읽히지 않아요.',
    removeYes: '지울게요',
    removeNo: '그대로 둘게요',
    missingTitle: '이 항목은 근거를 읽지 못했어요',
    missingLead: '파일이 옮겨졌거나 지워졌을 수 있어요.',
    reveal: '원본 위치 보기',
    actionFailed: '처리하지 못했어요.',
    settledApproved: '맞다고 확인했어요.',
    settledDeferred: '나중에 다시 보여드릴게요.',
    settledRemoved: '지웠어요.',
    done: '지금은 여기까지예요.',
  },
  today: {
  ...
  rules: {
    screenTitle: '규칙',
    waiting: '승인을 기다려요',
    waitingHint: 'AI가 자기 규칙으로 제안한 것들이에요. 승인하기 전에는 AI에게 전달되지 않아요.',
    active: '적용 중',
    activeHint: '대화를 시작할 때마다 AI에게 전달돼요.',
    none: '기다리는 규칙이 없어요. AI가 새로 제안하면 여기에 올라와요.',
    noneActive: '아직 적용 중인 규칙이 없어요.',
    approve: '승인',
    decline: '규칙이 아니에요',
    declineAs: '이렇게 남길게요',
    asPast: '기록으로',
    asState: '현재 믿음으로',
    cancel: '다음에',
    wroteBy: (source) => `${source}가 씀`,
    readWhole: '전문 보기',
  },
  register: {
    screenTitle: '기록된 값',
    intro:
      'AI가 주제마다 들고 있는 값이에요. 쌓이지 않고 앞의 값을 대체해요. 고치면 다음 대화부터 그 값을 읽어요.',
    fixHere: '말해서 고치기',
    subjectHint: '항목 하나에 값 하나예요. 이전 값은 아래에 그대로 남아요.',
    none: '아직 없어요. 시간이 지나면 바뀌는 사실이 생기면 여기에 적혀요.',
    always: '계속',
    keyCount: (n) => `항목 ${n}개`,
    byPerson: '직접 적은 값',
    byAgent: 'AI가 적은 값',
  ...
  chat: {
    ...
    amendUnknown: '이름을 알 수 없는 노트를 정정해요',
    editTarget: (title) => `「${title}」을 제자리에서 고쳐요`,
    editUnknown: '가져온 문서를 고쳐요',
    editKeeps: '노트 속성은 그대로 둬요.',
    ...
    doneRuleApproved: (title) => `「${title}」가 이제 규칙이에요.`,
    doneRuleDeclined: (title) => `「${title}」는 이제 규칙이 아니에요.`,
    readFrom: '이 노트들을 보고 답했어요',
    expired: '여기서 제안했지만 적용되지 않았어요.',
    unmapped:
      '제가 할 수 있는 건 노트 쓰기, 노트 바로잡기, 문서 고치기, 기록된 값 바꾸기, 기다리는 규칙 승인이에요. 이 중 무엇을 어느 노트나 값에 할지 말해 주세요.',
    ...
    failure: {
      ...
      quota: '요청이 거절됐어요. 요금제나 한도 문제라서 다시 눌러도 같아요.',
  ...
  onboarding: {
    ...
    intro: {
      ...
      lead: 'AI는 대화가 끝나면 다 잊어요. memex는 그 사이 알게 된 걸 노트로 남겨요. 다음 대화는 지난 대화가 끝난 자리에서 시작해요.',
      ack: '시작하기',
    },
    vault: {
      eyebrow: '노트',
      title: '노트는 당신 것이에요.',
      lead: '이 컴퓨터에 파일로 저장돼요. 어디에도 올라가지 않고, 언제든 폴더째 들고 나갈 수 있어요.',
      label: '저장할 폴더',
      exists: '이미 있는 폴더예요.',
    ...
    engine: {
      eyebrow: '이야기할 상대',
      title: '누구와 이야기할지 정해요.',
      lead: '노트를 쓰고 고치는 건 Claude Code나 Codex와의 대화로 해요. 설치와 로그인은 memex가 할게요. 비밀번호나 토큰은 memex에 저장되지 않아요.',
    },
    model: {
      eyebrow: '검색',
  ...
  settings: {
    screenTitle: '설정',
    intro: '앱 화면과 memex에 연결된 것들이에요.',
    appearance: '화면',
    appearanceHint: '이 컴퓨터의 화면 설정을 따르거나 한 가지 테마로 고정할 수 있어요.',
    system: '시스템 설정 따르기',
    light: '밝게',
    dark: '어둡게',
    language: '언어',
    groups: {
      appearance: { title: '화면과 언어', hint: 'memex가 어떻게 보이고 읽히는지 정해요.' },
      notes: { title: '노트와 검색', hint: '어떤 노트를 읽고 어떻게 검색할지 관리해요.' },
      ai: { title: 'AI', hint: '대화, 초안, 정리 작업에 쓸 AI를 정해요.' },
      connections: { title: '연결', hint: 'memex를 읽거나 기록할 수 있는 앱을 관리해요.' },
      data: { title: '데이터와 관리', hint: '로컬 볼트의 상태를 살피고 복구해요.' },
    },
    jobs: {
      chat: { name: '고치기', what: '앱 안의 대화에 써요.' },
      draft: { name: '초안', what: '승인 전에 보여줄 초안을 써요.' },
      sweep: { name: '훑기', what: '노트를 기계적으로 훑어요.' },
    },
    models: '작업별 모델',
    defaultOnly: '각 일에 기본으로 쓸 모델이에요. 한 번만 바꾸려면 그 자리에서 고르세요.',
    searchModel: '검색',
    searchModelHint:
      '단어가 정확하지 않아도 뜻으로 찾아줘요. 이 컴퓨터에 남고, 받고 나면 인터넷 없이 돌아가요.',
    sources: 'memex가 읽는 볼트',
    sourcesHint:
      '기본 볼트에는 새 노트를 저장해요. 다른 Markdown 폴더를 더하면 한 번에 같이 검색할 수 있어요.',
    primaryVault: '새 노트를 이곳에 저장해요',
    connectedVault: '연결한 볼트',
    addVault: '볼트 추가',
    disconnectVault: '연결 해제',
    disconnectHint: '폴더의 파일은 그대로 두고 memex 검색에서만 제외해요.',
    reindex: '다시 색인',
    reindexing: '다시 읽는 중…',
    indexDone: (changed: number, unchanged: number) =>
      `${changed}개 반영 · ${unchanged}개 변경 없음`,
    sourceFailed: '볼트를 변경하지 못했어요. 폴더가 연결되어 있는지 확인해 주세요.',
    ...
    vault: '볼트',
    vaultHint: '위 세 가지 뒤에 있는 목록과 정리 작업이에요.',
    apps: '다른 앱에서 연결하기',
  ...
  engines: {
    ...
    unreadable: '로그인 상태를 읽지 못했어요. 터미널에서 직접 로그인한 뒤 다시 시도해 주세요.',
    install: '설치하기',
    signIn: '로그인',
  ...
  links: {
    absent: '이 컴퓨터에 없어요.',
    ready: '아직 연결되지 않았어요.',
    elsewhere: (command) => `다른 memex(${command})에 연결돼 있어요.`,
    connect: '연결하기',
    repoint: '여기로 바꾸기',
  },
  connect: {
    screenTitle: '연결',
    appsTitle: '내 앱',
    intro: 'memex에 연결할 수 있는 앱들이에요. 연결하면 그 앱의 AI가 여기 기억을 읽고 써요.',
    connected: '연결됨',
    connect: '연결하기',
    repoint: '여기로 맞추기',
    restart: (name) => `${name}를 다시 켜면 적용돼요.`,
    notInstalled: '이 컴퓨터에 없어요.',
    absent: '아직 memex에 연결되지 않았어요.',
  ...
  history: {
    ...
    neverCommitted:
      '이 파일은 아직 버전 기록이 없어요. 기록이 생기면 그때부터 편집을 되돌릴 수 있어요.',
  },
  tree: {
    title: '노트',
    empty: '아직 노트가 없어요.',
  ...
  sidebar: {
    ...
    notes: '노트',
    state: '현재 믿음',
    agentShare: (n) => `그중 ${n}개는 AI 것`,
    rule: '규칙',
    rulesWaiting: (n) => `${n}개 승인 대기`,
    topics: '주제',
    threads: '정정 흐름',
    more: (n) => `${n}개 더 보기`,
  ...
  overview: {
    ...
    emptyLead: 'Claude와 이야기하면 memex가 채워져요. 앱을 연결하면 남길 만한 것이 여기에 쌓여요.',
    emptyAction: '앱 연결하기',
    title: '확인하기',
    ...
    stalenessHint:
      '선은 최근 1년의 주간 활동이에요. 오른쪽이 평평하면 손을 뗀 지 오래됐다는 뜻이에요.',
    stalenessShare: (pct) => `${pct}%가 지난 이야기`,
  },
  topic: {
    ...
    arcFallback: '아직 이어지지 않은 것',
    companions: '함께 붙는 주제',
    companionsHint: '같은 노트에 함께 달린 주제예요. 이 이야기가 어디로 흩어져 있는지 보여줘요.',
    sameThing: '· 같은 말 같아요',
    overlap: (pct) => `· ${pct}% 겹침`,
    all: (n) => `전체 ${n}`,
  },
  note: {
    ...
    borrowedNoClaude: 'Claude를 찾을 수 없어서 초안을 만들지 못했어요. 직접 고치시면 돼요.',
    borrowed: '이 파일은 직접만 고칠 수 있어요. AI는 읽고, 부탁하면 초안까지만 만들어요.',
    hideSource: '원본 닫기',
    sourceMissing: '목록에는 있는데 그 경로에 파일이 없어요.',
    agent: 'AI',
    correctedBy: (n) => `이후 ${n}개 노트에서 바로잡혔어요`,
    continuedBy: (n) => `이후 ${n}개 노트로 이어져요`,
    amendedBy: (n) => `이후 ${n}개 노트가 이 노트를 다뤄요. 정정인지 이어쓰기인지는 안 적혀 있어요`,
  ...
  tags: {
    ...
    overlapHint:
      '거의 같은 노트들에 함께 붙어 있어요. 같은 뜻이라는 증거는 아니니 직접 판단해 주세요.',
    merge: '합치기',
    merging: '합치는 중…',
    swap: '반대 이름을 남기기',
    ...
    unwritten: (n) => `${n}개는 파일에 태그가 없어서 다시 읽어 들이면 되돌아가요`,
    skipped: (n) => `볼트 밖 ${n}개는 건드리지 않았어요`,
    screenTitle: '태그',
    summary: (all, editable) => `${all}개 · 그중 ${editable}개를 고칠 수 있어요`,
    onceHint: (n) => `${n}개는 노트 하나에만 붙어 있어요`,
    onlyOnce: '한 번만 쓰인 것',
    filter: '태그 거르기',
    rename: '이름 바꾸기',
    renameHint: 'Enter로 적용해요. 이미 있는 이름을 쓰면 그쪽으로 합쳐져요',
    remove: '지우기',
    confirmRemove: (notes) => `노트 ${notes}개에서 뗄까요?`,
    outside: '볼트 밖',
    outsideHint: '가져온 문서에만 있는 태그라 고칠 수 없어요.',
  ...
  edit: {
    ...
    cancel: '취소',
    becomingPast: '기록이 되면 다시 편집할 수 없어요. 이후 수정은 정정 노트로 해요.',
    correctionTitle: '정정 노트 쓰기',
    correctionWhy: '일어난 일의 기록은 고쳐 쓰지 않아요. 정정은 이 노트를 가리키는 새 노트가 돼요.',
  ...
  evidence: {
    ...
    changed: '고른 뒤로 내용이 바뀜',
    missing: '볼트에서 사라짐',
    accounted: '이 근거들을 읽었어요',
  ...
  search: {
    ...
    recent: '최근 본 것',
    noTitleMatch: '제목에는 없어요. Enter를 누르면 본문까지 찾아요',
  },
  stale: {
    header: (n) => `이 노트를 마지막으로 손본 뒤 관련 노트 ${n}개가 쌓였어요`,
    noChangeTitle: '고칠 게 없어요',
    noChangeFallback: '새 노트들을 읽어봤지만 이 노트가 말하는 것과 어긋나는 게 없어요.',
    noChangeHint: '경고를 끄려면 「아직 맞아요」를 눌러 주세요.',
    unexplained:
      '바꾼 이유를 안 적었어요. 근거를 확인할 수 없으니 바뀐 부분을 직접 읽고 판단해 주세요.',
    whyTitle: '왜 이렇게 바꿨는지',
    noteRef: (id) => `노트 #${id}`,
    save: '저장',
    stillTrue: '아직 맞아요',
