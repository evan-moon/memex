...
const en = {
  ...
  today: {
    title: (n: number) => `${n} for today`,
    lead: 'Finish these and you are done. The rest can wait.',
    empty: 'Nothing for today.',
    emptyLead: 'Everything that was due has been answered.',
    now: 'now',
    soon: 'soon',
    evidenceMoved: 'A hypothesis whose sources moved',
    typoLink: 'A link that almost matches a note',
    typoHint: (target: string, nearest: string) => `"${target}" → "${nearest}"`,
    undeclared: 'A judgement that has not said what it stands on',
    undeclaredHint: (n: number) => `${n} candidates`,
    start: 'Start',
    buried: 'Put aside — click to see',
    buriedUndeclared: (n: number) => `${n} more to source`,
    buriedStale: (n: number) => `${n} maybe out of date`,
    buriedForward: (n: number) => `${n} links to notes not written yet`,
    buriedPlaceholders: (n: number) => `${n} links that were only examples`,
    buriedTagMerges: (n: number) => `${n} tags that may be one`,
    buriedLooseTags: (n: number) => `${n} tags used once`,
    dismiss: 'Ignore this note\u2019s unresolved links',
    dismissed: 'Put aside',
  },
  sidebar: {
  ...
  repair: {
    ...
    claims: (n: number) => `What this note asserts · ${n}`,
...
const ko: typeof en = {
  ...
  threads: {
    ...
    empty: '아직 다른 노트를 정정한 노트가 없어.',
    steps: (n) => `${n}단계`,
    branches: (n) => `${n}번 갈라짐`,
    span: (from, to) => `${from} → ${to}`,
    startsHere: '여기서 시작',
    alsoWent: '여기서 이쪽으로도 갔어',
    latest: '지금 여기까지 왔어',
    open: '이 줄기 전체 보기',
  },
  notFound: {
  ...
  today: {
    title: (n) => `오늘 ${n}장`,
    lead: '다 하면 끝. 나머지는 오늘 볼 필요 없어.',
    empty: '오늘 치가 없어.',
    emptyLead: '기한이 된 건 전부 답했어.',
    now: '지금',
    soon: '곧',
    evidenceMoved: '근거가 움직인 가설',
    typoLink: '노트 제목과 거의 같은 링크',
    typoHint: (target, nearest) => `"${target}" → "${nearest}"`,
    undeclared: '무엇에서 나왔는지 말하지 않은 판단',
    undeclaredHint: (n) => `후보 ${n}개`,
    start: '시작',
    buried: '묻어둔 것 — 눌러야 보임',
    buriedUndeclared: (n) => `근거 미선언 ${n}`,
    buriedStale: (n) => `아마 아직 맞음 ${n}`,
    buriedForward: (n) => `아직 안 쓴 노트로 가는 링크 ${n}`,
    buriedPlaceholders: (n) => `예시였을 뿐인 링크 ${n}`,
    buriedTagMerges: (n) => `같은 태그일지도 모르는 것 ${n}`,
    buriedLooseTags: (n) => `한 번만 쓴 태그 ${n}`,
    dismiss: '이 노트의 미해결 링크 전부 무시',
    dismissed: '묻어둠',
  },
  sidebar: {
    state: '지금 믿는 것',
