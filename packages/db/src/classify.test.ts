...
const VAULT = '/home/e/Second Brain';

const note = (over: Partial<ClassifyInput> = {}): ClassifyInput => ({
  filePath: `${VAULT}/a note.md`,
  title: 'a note',
  content: 'body',
  layer: 'past',
  tags: [],
...
describe('classifyNote', () => {
  it('takes a declared type over every rule', () => {
    const label = classifyNote(note({ declaredType: '규칙', title: '[정정] x' }), VAULT);
  ...
  it('reads a blog post off its path', () => {
    const label = classifyNote(
      note({ filePath: '/dev/evan-blog/content/posts/http3/index.md' }),
      VAULT,
    );
  ...
  it('calls anything else outside the vault a code document', () => {
    expect(classifyNote(note({ filePath: '/dev/memex/README.md' }), VAULT).type).toBe('코드문서');
  });

  it('reads a handoff off its title before its tags', () => {
    const label = classifyNote(
      note({ title: 'memex 세션 인계 2026-08-31', tags: ['memex'] }),
      VAULT,
    );
  ...
  it('reads an amendment off its title', () => {
    expect(classifyNote(note({ title: '[Amendment] x' }), VAULT).type).toBe('정정');
    expect(classifyNote(note({ title: '[정정] x' }), VAULT).type).toBe('정정');
  });

  it('reads a rule off its layer', () => {
    expect(classifyNote(note({ layer: 'rule' }), VAULT).type).toBe('규칙');
  });

  it('marks a tag-only match as weak', () => {
    expect(classifyNote(note({ tags: ['opula'] }), VAULT)).toEqual({
      type: '제품작업',
      area: '내 제품',
      method: 'tag',
      confidence: '약',
    });
    expect(classifyNote(note({ tags: ['1on1'] }), VAULT)).toMatchObject({
      type: '업무메모',
      confidence: '약',
    });
  });
  ...
  it('needs length as well as a closing heading to call something an essay', () => {
    ...
    expect(classifyNote(short, VAULT).type).toBe('미분류');

    const long = note({ content: `## 마치며\n\n${'긴 글 '.repeat(1000)}` });
    expect(classifyNote(long, VAULT)).toMatchObject({ type: '에세이', method: 'heading' });
  });

  it('falls back to 미분류 with weak confidence', () => {
    expect(classifyNote(note(), VAULT)).toEqual({
      type: '미분류',
      area: '기타',
      method: 'fallback',
      confidence: '약',
