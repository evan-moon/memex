...
//
// A fact has a truth value that can change: it was true in August and false in
// September. A judgement never had one — it was a view someone held. A rule is
// an instruction, and instructions are approved, not verified. Asking the deck's
// question of the last two produces a card nobody can answer, which is how a
// review queue teaches people to dismiss it.
export type ClaimKind = 'fact' | 'judgement' | 'rule';
...
export const classifyClaim = (text: string): ClaimKind => {
  ...
  return EVALUATIVE.test(trimmed) ? 'judgement' : 'fact';
};
