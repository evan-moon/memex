...
export const listDeferrals = (client: MemexClient): Deferral[] =>
  (client.sqlite.prepare('SELECT * FROM review_deferrals ORDER BY at DESC').all() as Row[]).map(
    toDeferral,
  );
...
export const wakeDeferrals = (client: MemexClient, states: ReviewState[]): Waking => {
  ...
  const asleep = new Set(
    held
      .filter((deferral) => staysAsleep(deferral, byKey.get(deferral.itemKey)))
      .map((deferral) => deferral.itemKey),
  );

  const waking = held.filter(
    (deferral) =>
      deferral.wokenAt === null && !asleep.has(deferral.itemKey) && byKey.has(deferral.itemKey),
  );
  const now = Date.now();
  const mark = client.sqlite.prepare('UPDATE review_deferrals SET woken_at = ? WHERE item_key = ?');
  ...
  return {
    asleep,
    met: new Set(
      held.filter((d) => !asleep.has(d.itemKey) && byKey.has(d.itemKey)).map((d) => d.itemKey),
    ),
  };
};
